...
export class GameController extends Component {
    ...
    init() {
        ...
        this.piece_count = 0;
        this.bubble_count = 0;
        this.level_big_piece = instantiate(this.level_prefabs[this.level % this.level_prefabs.length]).getComponent(
            BigPiece
        );
    ...
    setCoin(amount: number = this.coin, isSetLabel = true) {
        ...
        if (isSetLabel) this.UIMainScreen.setCoinLabel(this.coin);
    }
        ...
        if (bubble && !bubble.isPop && bubble.isPopable) {
            ...
            if (this.bubble_count == this.level_bubble_count) {
                if (this.gamemode == GameMode.NORMAL) {
                    ...
                    this.bubble_count = 0;
                    if (this.level_big_piece.TurnTimeCount == this.level_big_piece.MaxTurnTime) this.winBonus();
                    else
                        this.scheduleOnce(() => {
    ...
    onTouchEnd() {
        if (this.gamestate == GameState.MATCH_PIECE && this.isHit) {
            ...
            if (this.piece_count == this.level_piece_count) piece.drop(false);
            else piece.drop(true);
            this.objectHit = null;
            this.isHit = false;
            this.hit_diff = null;
    ...
    playAudio(type: SoundType, delay = 0) {
        ...
        switch (type) {
            case SoundType.POP:
                clip = this.pop_sounds[Math.floor(Math.random() * this.pop_sounds.length)];
                break;
            case SoundType.MATCH:
                clip = this.match_sound;
                break;
            case SoundType.MATCH_COMPLETE:
                clip = this.match_complete_sound;
                break;
            case SoundType.LEVEL_COMPLETE:
                clip = this.level_complete_sound;
                break;
        }
        ...
        } else this.audioSource.playOneShot(clip, 1);
    }
